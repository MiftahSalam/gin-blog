...
var MockFavoriteArticle = []RouterMockTest{
	{
		UserMockTest: userServices.MockTests{
			TestName: "error (article not found): Favorite Article Test",
	...
	{
		UserMockTest: userServices.MockTests{
			TestName: "error unauthorized (no token present): Favorite Article Test",
			Init: func(req *http.Request) {
				req.Header.Set("Content-Type", "application/json")
			},
	...
	{
		UserMockTest: userServices.MockTests{
			TestName: "no error: Favorite Article Test",
			Init: func(req *http.Request) {
				req.Header.Set("Content-Type", "application/json")
				req.Header.Set("Authorization", fmt.Sprintf("Bearer %v", common.GetToken(userModels.UsersMock[0].ID)))
			},
